...
function printHelp(): void {
  process.stdout.write(
    [
      ...
      "  safeinstall decisions <verify|authorize> --base <ref> [--head <ref>] [--allow-registry <url>] [--output <file>]",
      ...
      "               never trusted.",
      "  decisions authorize  decisions verify plus a fresh policy re-evaluation",
      "               of the committed head state with registry metadata fetched",
      "               now (the L1 gate). Refuses dirty working-tree state.",
      "               --output writes the authorization artifact (canonical",
      "               JSON) for CI upload.",
      "  decisions attest  Build the signable in-toto statement (the DSSE",
      "               payload) over an authorization artifact. Sign it with",
      "               Sigstore keyless in CI (OIDC) — the statement alone is",
      "               not a signature.",
      "  decisions verify-attestation  Check that a statement binds a given",
      "               authorization artifact (subject digest + verdict + head).",
      "               NOT signature verification — that is the Sigstore-bundle",
      "               step.",
      "",
      "Global options:",
